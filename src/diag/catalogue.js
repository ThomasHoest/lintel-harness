 ...
 * nobody looks for.
 */
export const DESCRIPTIVE_SLOTS = {};
/** Placeholder names a template expects, in order of first appearance. */
export function placeholdersOf(code) {
    const seen = new Set();
...
export function missingPlaceholders(code, data) {
 ...
 */
export function render(code, data = {}, lists = {}) {
    const out = [];
    for (const line of MESSAGES[code]) {
        ...
        let had = false;
        const filled = line.replace(PLACEHOLDER, (whole, name) => {
            had = true;
            const v = data[name];
...
export function renderText(code, data = {}, lists = {}) {
