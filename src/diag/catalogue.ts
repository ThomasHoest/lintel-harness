...
export const MESSAGES: Readonly<Record<DiagnosticCode, readonly string[]>> = {
  ...
  'E-MANIFEST-NEWER': ["lintel: .harness/manifest.json was written by a newer lintel (manifest version {n}; this CLI reads up to {m}).", "  → Upgrade: npm i -g @linteldk/cli@latest"],
  ...
  'E-PACK-CLI-TOO-OLD': ["lintel: pack {name}@{version} needs lintel {minCliVersion} or newer.", "  You are running {cliVersion}.", "  → Upgrade with: npm i -g @linteldk/cli@latest"],
  ...
  'E-UPDATE-NOT-NEWER': ["lintel: {pack}@{applied} is applied; lintel {cliVersion} bundles {bundled}, which is not newer.", "  → Upgrade the CLI: npm i -g @linteldk/cli@latest"],
  ...
  'E-WRITE-FAILED': ["lintel: could not write \"{path}\" ({errno}).", "  Nothing further was written; the project is mid-apply.", "  → lintel harness {command} --rollback"],
