...
| **1.0.1** | **2026-09-01** | **Q-63 — binary rename.** The binary becomes **`lintel`** with **`harness` as a command group**, and the published package becomes **`@linteldk/cli`**; every usage line in this document is now `lintel harness <command>`. Q-16 is **amended, not superseded**, and its §Resolved Decisions row keeps the original terms with a pointer to Q-63; the new row for Q-63 is added beside it. **Nothing else moves:** the command surface is the same size, F1 remains the only message catalogue, no exit class changes, and the F1-side detail — the `lintel:` diagnostic prefix, the re-scoping of `E-CLI-UNKNOWN-COMMAND`, and the missing group-level code recorded as F1 known limit 16 — lands in F1 v2.8 rather than here. **Not folded here and still stale:** the *Release gate* and *v1.0 command surface* rows predate **Q-62**, which returns `update` to v1.0 as F3's. |
...
| Package, binary, runtime floor | Published as `@linteldk/cli`; binary `lintel`; **command group `harness`**; `engines.node >= 22` | Q-16 · Q-63 |
...
into `@linteldk/cli` or loaded as plugins — and are recorded in the
...
| Q-16 | Published as `@lintel/harness`; binary `lintel-harness`; Node >= 22 — **amended by Q-63**, which makes the package `@linteldk/cli`, the binary `lintel` and `harness` a command group. The original terms are kept here because Q-63 amends Q-16 rather than replacing it | brief §12 |
...
| Q-63 | **The binary is `lintel`, with `harness` as a command group: `lintel harness init coding`. The package is `@linteldk/cli`.** Amends Q-16. **Deliberately not decided (Q-64):** whether later tools are built into `@linteldk/cli` or loaded as plugins | brief §12 |
