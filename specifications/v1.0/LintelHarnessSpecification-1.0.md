...
over seven fixed primitives — `copy`, `rename`, `strip-suffix`,
`rewrite-path`, `substitute`, `generate`, `merge-json` — applied
...
model, because path confinement, the reserved-destination denylist and
consent gating all depend on the plan being inspectable **before**
...
| Document | Covers |
|---|---|
| [`../general/pack-application.md`](../general/pack-application.md) | The two-phase apply — the phase table, the init flowchart, the seven-primitive recipe set, the determinism requirement, and what the coding pack's recipe encodes |
...
| Phase 2 form | A **declarative recipe** over seven fixed primitives — `copy`, `rename`, `strip-suffix`, `rewrite-path`, `substitute`, `generate`, `merge-json` — applied automatically by the CLI, never by the user. A new kind of step requires a new primitive in the CLI, deliberately | Q-40 · `general/pack-application.md` |
...
| Hooks | **No pack may register a hook at v1.0.** `planning` ships one guard script inert at `0644`, registered by nothing; `writing` ships none | Q-30, Q-45 · `general/pack-inventory.md` |
...
| Q-23 | `merge-json` is kept and tightened: the only mode permitted on a JSON target, with declared owned keys | F1 §Resolved Decisions |
