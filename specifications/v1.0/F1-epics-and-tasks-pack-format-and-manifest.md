...
| 4.4 | 2026-09-02 | **`verify` is wired, the stub set is empty, and all six commands work.** It was last on purpose: the shaping functions existed since T-1002, and the command layer is the part that is **work rather than wiring** — it must recompute from **`.harness/pack/`**, never from the pack this CLI bundles. A `verify` reading the bundled pack would **pass a project whose committed payload had been swapped**, because it never looked at it, and **fail a project applied correctly by an older CLI**, because the bundle has moved on. Both silently. So `read-applied.ts` exists to make the input explicit — payload, recipe and declarations all from the committed copy — and the property is asserted rather than argued: a test **edits the committed recipe so that it disagrees with the bundled pack**, re-records the digest so the gate passes, and requires `verify` to follow the committed one. **A `--json` asymmetry is recorded, not papered over**: C-29's read-back check runs in **two** places, `readManifest` and `verifyProject`'s second gate, and the first wins — so a digest failure emits a document and an answer failure does not, where F3's US-71 reasons a suppressed classification should stay distinguishable *through* the document. Closing it means deciding which check owns the fault. **The stub note in `main.ts` is now dead code**, and the tests that watched the set shrink assert its emptiness rather than being deleted with it: a seventh command added without a dispatch would land back on the note. **238 integration tests.** |
| 4.3 | 2026-09-02 | **`validate` and `pack info` reach argv, and T-1214's gate runs in CI.** Both had command modules — `runValidate`, `renderPackInfo` — that were built, tested and **called by nothing**, so the CLI answered *"is not implemented in this build"* for two commands that were. That is the same disagreement `main.ts` already carried once, when its stub predicate tested **ownership** rather than whether a command was built; the predicate tests builtness now and a test holds the set. **`verify` is deliberately left out**: its shaping functions exist, but its command layer has to recompute from `.harness/pack/` rather than from a plan it was handed — which is the property that makes `verify` an *independent* check rather than a restatement — and claiming it here would be the same lie in a new place. **Found by running it:** every finding printed **twice**, because `summaryLines` already interleaves each pack's findings with its own summary line and the command reported the bag as well. Both halves were individually correct, which is exactly why only running it showed the fault. **T-1214 is discharged and CI now runs the gate as its own step** — redundant with `ci-gate.test.ts` on purpose: the test proves the property, the step makes the failure legible as *the bundled packs no longer pass their own gate* rather than as one of a thousand tests breaking. |
...
| 2.9 | 2026-09-02 | **T-0307 — the anatomy, and Q-60 validated end to end.** Three faults that look alike are kept apart: **missing** (absent, or present and naming no content — G-F1-3's *silently omits one*), **empty** (globs matching no payload file), and **contradiction** (content beside `"status": "absent"`, which is explicitly **not** the unknown-key rule — those keys' *meanings* collide). And two that are not faults: `W-ANATOMY-ABSENT` and `W-ANATOMY-PROVISIONAL` are **`notice`**, so `--strict` does not promote them. **A test proves what that buys**: `planning` declares a provisional role set on purpose (Q-11) **and still exits 0 under `--strict`** — Q-60's whole reason for existing, asserted against the real pack rather than a fixture. The redundant-key case is deliberately the other way — **`defect`**, since the key is ignored and the author is expected to delete it, unlike the status beside it. **All three packs have a complete, non-empty anatomy.** **163 unit tests.** |
| 2.8 | 2026-09-02 | **T-0304 and T-0306 — semver, and the glob dialect decided.** T-0306 delegated a decision (*"choose the smallest dialect … and state it"*), and it was settled by **survey rather than taste**: across all three bundled packs, **`*` is the only non-literal character any glob contains**. So the dialect is `*` within one segment, everything else literal, **no `**`** — and the asymmetry is what decides it, since adding `**` later is additive while withdrawing it would break packs. Folded into F1 **v4.4** and the ⚠️ register, because U-3 asked for it stated rather than merely implemented. **A test asserts no bundled pack uses a construct outside the dialect**, so the survey cannot silently stop being true. The matcher is a **two-pointer walk, not a compiled `RegExp`** — an author-supplied pattern is untrusted input, and compiling one would reintroduce the risk C-7 bounds for `pattern`. Semver is parse-and-compare only: **`minCliVersion` is a floor**, so range arithmetic would be machinery for a question the format never asks. **150 unit tests.** |
...
- [x] **T-0211** `[Implementer]` Add **`skills`** to the
...
- [x] **T-0301** `[Implementer]` `src/json/parse-strict.ts` — `parseStrictJson()`,
...
- [x] **T-0303** `[Implementer]` `src/pack/schema.ts` — the hand-rolled
  ...
  **boolean-typed field** — the list **closed at five**, `RecipeStep.executable`,
  `RecipeStep.adaptExpected`, **`RecipeStep.fillExpected`** (Q-79),
  `ParameterDecl.required`, `ParameterDecl.notASecret`
...
- [x] **T-0304** `[Implementer]` Semver parse and compare, used by
...
- [x] **T-0305** `[Implementer]` `src/pack/load-pack.ts` — resolve `packs/<name>/`
...
- [x] **T-0306** `[Implementer]` `src/recipe/glob.ts` — **the one bounded glob
...
- [x] **T-0307** `[Implementer]` `src/pack/anatomy.ts` — the nine-part
...
- [x] **T-0308** `[Implementer]` `src/pack/parameters.ts` — declaration rules and
...
- [x] **T-0309** `[Implementer]` `src/security/secret-heuristic.ts` — the
...
- [x] **T-0310** `[Implementer]` `src/pack/scaffolds.ts` — selection by id
...
- [x] **T-1004** `[TestWriter]` Acceptance tests for US-33 in
...
- [x] **T-1005** `[TestWriter]` Acceptance tests for the fill-expected states
...
- [x] **T-1214** `[TestWriter]` The **CI gate**: `lintel harness validate --all
