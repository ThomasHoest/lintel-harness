...
| Streams | Diagnostics and warnings to **stderr**; the disclosure, prompts' echo and the post-apply summary to **stdout** | *(IM-10)* IM-10's required test captures the disclosure as a contiguous substring, which needs a stream a caller can capture without interleaved diagnostics |
...
**Acceptance criteria:**
- On every apply that reaches the lock, `init` renders the disclosure to **stdout**, as one contiguous block, **before** `.harness/lock` is taken and therefore before any byte is written.
