...
**Status:** Accepted
**Date:** 2026-09-01
**Platform:** **Markdown under `.claude/skills/`, and nothing else** — no runtime, no build, no package, no dependency and no tests of its own (`general/technology-choices.md` §9). It ships in `@linteldk/cli` with the binary it drives (`lintel`, command group `harness` — Q-63). The surfaces it acts on are a conversation and a filesystem.
...
| **`lintel` not installed, or the command not found** | Say so and stop. Do **not** perform the apply by hand, copy pack files, or write a manifest (IM-5). The remedy is installing `@linteldk/cli`, and the skill says so without inventing an install command F2 has not specified |
