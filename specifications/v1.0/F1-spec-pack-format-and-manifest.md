...
**Status:** Accepted
**Date:** 2026-09-01
**Platform:** Node ≥ 22 / TypeScript CLI, published as `@linteldk/cli`, binary `lintel`, with **`harness` as a command group** — every command in this document is reached as `lintel harness <command>` (Q-16 **as amended by Q-63**). Pack content is Markdown, shell, PowerShell and Bicep; `pack.json`, `recipe.json` and the manifest are JSON. No UI.
...
| **6.2** | **2026-09-02** | **The exemption T-0211 states in prose now has a constructor, and `--version` exists.** **(a)** T-0211 says *"`skill install` is a CLI write and is unaffected — the reservation binds recipe steps"*, and **no constructor expressed it**: `confinePath` refuses `.claude/skills/` (C-53) and `harnessPath` admits only `.harness/**`, so `skill install` could be typed only by casting past the brand — which would have made it unjournalled, unrollbackable and invisible to every guard. `harness-paths.ts` gains **one** CLI-owned location outside `.harness/`, `.claude/skills/lintel/`, minting `HarnessPath`: the brand means *a CLI write*, and the `.harness/` prefix was a property of the list rather than of the brand. **A recipe step still cannot reach it** — `confinePath` refuses it exactly as before, which is C-53 intact. `WriteRequest.command` and `JournalCommand` gain **`'skill'`** for the same reason they are non-defaultable: the remedy is rendered from them. **(b) `lintel --version` did not exist**, and F6's skill opens **every flow** with it — so the skill halted at step 0 of every task it has. Answered before the group is parsed, printing the bare version on stdout, because a handshake that required knowing the command vocabulary could only be performed by someone who already understood the CLI. **(c) `package.json` did not publish `skill/`**, so `skill install` would have found nothing in an installed `@linteldk/cli`. **(d) A failed run left its lock behind.** Keeping it is right **mid-apply** — releasing it would invite a second writer into a half-written project (US-49) — and wrong when the run stopped **before the journal**, where it stranded the project for the stale timeout over a fault one `rmdir` fixes. `ExecuteResult` gains `journalled`, because `complete: false` alone cannot tell the two apart. Removing the empty `.harness/` with it **closed a carve-out E-22 had to record**: *"init writes no applied path except through `executeApply`"* is now simply *"the project directory is untouched"*. |
| **6.3** | **2026-09-02** | **A remedy that performed the thing it offered to show, and a flag the parser refused while the catalogue advised it.** Both found by building E-25. **(a) `E-UPDATE-AVAILABLE` rendered `→ lintel harness update   to see what would change`** — and **`update` writes**; it is `--dry-run` that shows. So a user who ran `--dry-run`, read that arrow and followed it **performed the apply they were inspecting**. Not a stale remedy but an actively harmful one: it converts a deliberate look into a write, at the exact moment the user has signalled they are not ready. Corrected to `--dry-run`. **(b) `update --rollback` was unreachable from argv** while `E-JOURNAL-PRESENT` renders `→ lintel harness update --rollback` **from the journal's own `command` field** — so after a crashed update the CLI printed a remedy and then refused it, naming `init`, the command that answers `E-ALREADY-APPLIED`. F3 §F3.2's header line, US-66 and F3-R3 all require the flag; the branch was written and tested behind it. **This is the fifth instance of one fault** — after `E-JOURNAL-PRESENT` (v2.9), `W-ROLLBACK-KEPT` (v5.5), `E-UPDATE-PARAM-UNANSWERED` (v5.6) and `E-TARGET-RACE`/`E-WRITE-FAILED` (v5.9) — and **the first where the CLI printed the unusable remedy itself** rather than a document describing one. Catalogue unchanged at **91**. |
| **6.4** | **2026-09-02** | **The CLI did nothing at all when installed, and every test passed.** `package.json`'s `bin` becomes a **symlink** at `node_modules/.bin/lintel`, and the entry-point guard compared `process.argv[1]` against `import.meta.url` **as strings** — two spellings of one file, the first the link and the second already resolved. So `startedByNode` was false on every installed invocation and the process exited **0 with no output**: silence indistinguishable from success. **Nothing caught it because every test in this repository runs the file by its real path** — from source, from `dist/`, or spawned with an absolute path — which is the one arrangement where the two spellings agree. **The defect lived exactly in the gap between how the product is tested and how it is used**, and only `npm install` reached it. Compared as resolved real paths now, with an integration test that installs the package and drives the shim — the only shape that would have failed. Found by asking what a user needs in order to try this, and then trying it. Catalogue unchanged at **91**. |
| **6.5** | **2026-09-02** | **Known limit 23 is closed: `package.json` is `1.0.0` and agrees with `CLI_VERSION`.** The obvious follow-on was to delete the constant and read the version from `package.json`, and it is **not** taken — the reasoning `version.ts` already carried survives the numbers agreeing: *a version the product must find on disk is a version that can go missing in a bundler's output, and every version gate then fails open or closed depending on how the read was written.* Two numbers kept equal by a test is a smaller risk than one number resolved at runtime, and `tests/integration/tarball.test.ts` is that test. **Limit 21 stands** — the permission pin still records no runtime version, and `SECURITY.md` still calls it a release blocker. |
| **6.6** | **2026-09-02** | **The disclosure containment check was wired into `init` and `update` and not into `validate`.** T-0806 says it plainly — *"`validate` runs the same refusal at **step 11** over the rendered set. No fifteenth validate step"* — and `scanForForgery` was called from `consent.ts` alone. So **`validate --all --strict` exited 0 on a pack `init` would refuse**: the gate giving a clean bill for something it never checked, and `validate` is precisely what a pack **author** runs and what CI runs **before anybody applies the pack**. This is the CRITICAL that took **four Mode A rounds** to close — a pack whose content carries a line shaped like the end delimiter can make a reader believe the disclosure finished before the interesting rows — and it had a hole in the one place a pack is examined before it is used. Wired at step 11, over the same disclosure object `pack info` renders and `init` shows, with **no nonce**: `buildDisclosure` is deterministic by C-62 so `pack info --json` stays byte-identical, and the shape rule is the half that applies to a pack sitting in a repository. **Found by an adversarial fixture written to the rule rather than to the code** — it was authored skipped, with the finding in its skip reason and an instruction for what to do when it started failing. It started failing; the instruction was followed. Catalogue unchanged at **91**. |
| **6.7** | **2026-09-03** | **Known limit 21 closed — by *observing* the version, which is the thing nobody had done.** The pin recorded `version: null` because no document in this project named a Claude Code version, and an invented one would answer *"is this pin current?"* wrongly and with full confidence. **The gap was never "someone must decide a number" — it was "nobody has looked"**: `claude --version` in this repository reports **2.1.257**, now recorded with its source in the pin's `observedIn`. The guarding test kept its job and changed its shape: it refused a *guess* when the value was `null`, and now refuses a **version that arrives without evidence** — `observedIn` must name it. The re-check that came with the stamp found a near miss worth recording: **`dontAsk` is a real permission mode this pin has never named**, it is widening, and it is refused — **by the fail-closed rule rather than by an entry**, which is the asymmetry working: a mode the pin has not heard of costs a refusal, never a silent widening. Obligation 13 is restated as *move `version` and `takenOn` together*, because a version stamped without a re-check claims a check that did not happen and is worse than `null`. |
| **6.8** | **2026-09-03** | **The *"remedy that cannot work"* class, instances six, seven and eight — and the discovery that the guard written to catch the class was blind to exactly it.** (a) **`E-CLI-UNKNOWN-COMMAND` ended `→ lintel harness --help`, and that command did not exist**: typing it produced the same error, whose remedy was to type it again. (b) **`E-CLI-UNKNOWN-FLAG` ended `→ lintel harness {command} --help`, and command-level help existed for none of the six.** Three shapes of wrong, and the third is the serious one: `--help` was *refused* by `update` and `skill`, *diverted* by `init` ("needs a pack name" — a rule about arguments, answered to a request to be told the rules), and **obeyed** by `validate` and `verify`, where it parsed as no flag at all and the command **ran**. Asking `validate` what it does validated a pack and exited 0. **Help that silently does the work is worse than help that errors**, because nothing tells the user it happened. Both fixed by making the remedies *true* rather than by rewording them — group help and per-command help now exist, answered ahead of the flag parser, listing each command's accepted flags from the same table the refusal reads. (c) **The guard could not see either.** `tests/structural` matched `lintel harness ([a-z]+)`, so `--help` (starts with `-`) and `{command}` (starts with `{`) **both failed to match and were skipped, flags included** — the guard reported clean while the class it names was live in two places. Widened to both shapes, and a slot remedy must now hold for **every** command it can render as. (d) That widening immediately found **instance eight**: `{command} --rollback` renders from a journal whose `command` can be `skill`, and `skill install` refuses the flag — the same fault as instance five, one command over. It is a **real** gap, already documented as gap 3 in `skill.ts` (`skill install` writes no journal, so there is nothing to roll back and a `--rollback` mode would be a mode that lies), so it is recorded as an **explicit, explained exemption asserted in both directions** — unexplained fails, and an exemption nothing reaches any more also fails. Closing it is F6 work. **The through-line: every one of these was found by *running* the remedy rather than reading it.** |
| **6.9** | **2026-09-03** | **Trojan Source — the C-50 carriers, reached by a character class nobody had enumerated.** The Mode B implementation review (`security-review-mode-b-implementation.md`) ran the escaper rather than reading it and found that **bidirectional controls and zero-width characters passed through both `escapeLine` and `escapeValue`, and through `confinePath`'s grammar** — while ANSI, NUL and U+2028 were caught by all three. **They are not control codes**, which is the whole reason: every class in the product was written as a range over C0/DEL, and U+202E sits outside it. A step writing `scripts/setup\u202Egnp.sh` puts a **shell script** on disk while any bidi-aware terminal renders `scripts/setuphs.png` — so the **disclosure**, whose entire content is *"here is every path I will write"*, would have shown the user an image. Fixed in **both** places at once, mirroring C-50's split exactly rather than inventing a new posture: **escaped** in a parameter prompt and in verbatim frontmatter, which are prose the CLI must still print; **refused** at stage-1 grammar in a destination, where T-0113's own reasoning already says the grammar is *a stronger control than escaping* and no legitimate use exists. The set is one named constant, `BIDI_AND_INVISIBLE` — U+061C, U+200B–U+200F, U+202A–U+202E, U+2066–U+2069, U+FEFF — shared by both modules so they cannot drift. **Four fixtures added** (prompt, frontmatter, destination, zero-width destination), taking the adversarial suite from 60 to 64. **MEDIUM, not HIGH, and the reason matters**: `packDir()` resolves from the install directory and never `process.cwd()`, so no attacker-supplied pack can be applied at v1.0 — the gap becomes live with `contribute` (F4), which is the release it had to be fixed by. |
...
|---|---|
| `E-PACK-CLI-TOO-OLD` — pack requires a newer CLI | Exit 1. `lintel: pack {name}@{version} needs lintel {minCliVersion} or newer.` / `  You are running {cliVersion}.` / `  → Upgrade with: npm i -g @linteldk/cli@latest` |
...
| `E-MANIFEST-NEWER` | Exit 2. `lintel: .harness/manifest.json was written by a newer lintel (manifest version {n}; this CLI reads up to {m}).` / `  → Upgrade: npm i -g @linteldk/cli@latest` |
...
| `E-UPDATE-AVAILABLE` — a newer version of the applied pack is bundled (F3) | **Exit 1, and it is not a failure of the run** — it is the answer. `update`'s read-only mode (the former `status`) reports it and exits 1 so that a CI job can gate on "this project is behind" without parsing output. `lintel: {pack}@{applied} is applied; lintel {cliVersion} bundles {bundled}.` / `  → lintel harness update --dry-run   to see what would change` **The exit class is deliberate and is the one place in this catalogue where exit 1 reports a question answered rather than a fault.** F3 owns when it fires. |
| `E-UPDATE-NOT-NEWER` — the bundled pack is not newer than the applied one (F3) | Exit 1. `lintel: {pack}@{applied} is applied; lintel {cliVersion} bundles {bundled}, which is not newer.` / `  → Upgrade the CLI: npm i -g @linteldk/cli@latest` **`update` never resolves "newer" over the wire** (§NFR *No network*): a project moves to the version bundled in the installed CLI, so the remedy for "nothing newer" is upgrading the package. Covers both equal and older, because the remedy is the same and the two are not separately actionable. |
...
- **Hashing algorithm.** SHA-256, lowercase hex, all 64 characters, no
  ...
  (US-10, Q-52).
- **Zero runtime dependencies** (Q-81). The published `@linteldk/cli`
...
| 21 | **~~The `.claude/` permission pin has no runtime version recorded against it~~ — CLOSED at v6.7 (2026-09-03)** | §US-3 and §F1.9 obligation 13 both require *"the runtime version the pin was taken against recorded beside it"*. The implementation recorded `null` for as long as no document in this project named a Claude Code version — an invented string would have answered *"is this pin current?"* wrongly and with full confidence, which is worse than admitting the gap. Found by building T-0802 (v5.4). **Closed by observation, not by decision**: `claude --version` reports **2.1.257**, recorded with its source in `observedIn`, and the guarding test now refuses a stamped version whose evidence is missing. The accompanying re-check found **`dontAsk`** — a real, widening mode the pin has never named, refused by the fail-closed rule rather than by an entry |
