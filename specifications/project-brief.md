...
| Q-82 | **Add-on packs become a v1.1 feature (F7), and `coding`'s two backend scaffolds leave the pack for `addons/` now.** An **add-on** is a small composable unit that is *not* a way of working: infrastructure kits (`backend-azure`, `backend-aws`) and, at v1.1, `presentation`. A project holds **exactly one pack** and **zero or more add-ons**. Nothing applies an add-on at v1.0 — the mechanism is undesigned and deferred. | 2026-09-01 | The one-pack invariant is right for *ways of working* — two packs contradict each other — and wrong for *content bolted onto* one, which is what a backend kit or a presentation layer is. Modelling those as pack scaffolds forced them to live inside whichever pack happened to want them, so `coding` owned two cloud kits that have nothing to do with a specification process, and `planning` or `writing` could not have one without duplicating it. **Q-28 already sensed this** by deferring `presentation` as a "shared capability" with no home; add-ons are the home. **Moved now rather than at v1.1** because leaving them in `coding` would ship a v1.0 pack whose shape the v1.1 model contradicts, and because the recipe branches are far easier to preserve than to reconstruct — each add-on carries its old steps as a **record `recipe.json` that no CLI reads**. **`addons/` is deliberately not under `packs/`**: `packs/` means *what ships at v1.0*, and `validate --all`, `find packs -name pack.json` and F5's E-13 all rely on that. **Accepted cost, and it is real:** `writing-workstream` becomes the **only** scaffold in the product, so **scaffold exclusivity and the executable-bit rules lose their only bundled-pack subject** — all four v1.0 executables were in these two kits. Both remain covered by adversarial fixtures, and F1's fixture suite is now the *sole* coverage for two security-relevant rules, which is recorded rather than discovered later. **Supersedes Q-17's count** (three scaffolds at v1.0, now one) and **narrows Q-8/Q-8a**'s cloud framing to add-on content. |
