...
| # | Question | Notes |
|---|---|---|
| Q-60 | **Can `validate --all --strict` ever exit 0, and does `pack.json` carry a `provenance` key?** | Raised by building the `planning` pack. **G5.11 requires `validate --all --strict` to exit 0 over the three bundled packs, and two of them necessarily emit warnings by design.** `planning` emits `W-ANATOMY-PROVISIONAL` (part 2 is provisional — a recorded decision, not a defect) and `W-HOOK-SCRIPT-INERT` (the inert guard script, which exists *because* no pack may register a hook); `coding` emits `W-FOLDER-README-MISSING` for the intermediate `infrastructure/`, which US-16 step 12 requires and F1 §F1.3's own recipe does not write. So either `--strict` promotes only specific warnings — `W-FOLDER-README-MISSING` and `W-LINK-DANGLING` are the candidates — or the CI criterion is unachievable for the packs it exists to check. **Second half:** F5's §NFR *Provenance* requires each `pack.json` to record what the pack derives from; F1 defines no such field, so it lands as an unknown top-level key, which US-1 makes a warning — compounding the same problem. `provenance` is shipped in all three packs because F5 is explicit; F1 owns whether to define it. |
| Q-61 | **Is `coding`'s `CLAUDE.md` the only `generate` step in v1.0?** | F1 US-32 says it is; F5's planning 7b row and US-38 (*"each pack's generated `CLAUDE.md` contains anchors"*) say otherwise. The packs were built per F5 — `planning` has a `generate` step and seven anchors. One of the two statements has to go. |
| Q-56 | **Does an F6-adapted `CLAUDE.md` make `verify` fail, and is that acceptable at the release gate?** | Raised by `general/system-architecture.md`, the first document to view the whole system at once. F1 US-33 makes a green `verify` **the acceptance test for S7**; `verify` compares **whole files** (`match \| differs \| missing`) and region anchors are inert at v1.0, so nothing separates pack-owned regions from project-owned prose. The master spec's F6 section says the skill adapts that prose. **Both cannot hold**: the moment F6 does its stated job, `CLAUDE.md` reports `differs` and `verify` exits 1 with `E-VERIFY-MISMATCH`. F1 excuses a deliberate edit in prose (which is why it is exit 1, not 2), but S7 asserts a green `verify` as a gate. **Q-57 makes this the normal path, not an edge case.** Candidate resolutions: S7's acceptance is `verify` green *before* the skill runs; or `verify` gains a per-path allowance the manifest records; or F6's `CLAUDE.md` adaptation defers to v1.1 with the region parser. Owned by F6/F2, neither of which has a spec. |

Q-1…Q-55 and Q-57…Q-59 are resolved. Next free is **Q-62** — see §12. New questions raised
during specification keep the next free ID (**Q-56**…).
