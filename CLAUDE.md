...
| Versioning | Per-pack semver in `packs/<name>/pack.json` plus a declared `minCliVersion`, and a separate CLI semver; the manifest records both. All three packs declare `minCliVersion: 0.1.0` — lowered from `1.0.0` when the first release shipped as **0.1.0**, because a CLI below every pack's floor refuses every pack. |
