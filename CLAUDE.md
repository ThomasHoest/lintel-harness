...
| Error code | — | F1's catalogue is the **only** one, and it holds **89** at v4.0. No other document may invent a code. Eleven were added: nine at v3.0, **`E-DISCLOSURE-FORGERY`** at v3.4 closing the Mode A CRITICAL, and **`E-PACK-INVALID`** at v4.0 — found by *building* the strict reader, when `pack.json` turned out to have no code for a syntax fault while the other two documents that reader parses each had one. |
