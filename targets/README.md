...
| Template · Run.md · this README | `targets/` (reference) | source/scaffolding, copied per-goal |
...
`.claude/agents/`, the `/target` command in `.claude/commands/`, and this
reference pack (`target.template.md`, `Run.md`, this README) at `targets/`,
with the path references pointing at `targets/…`.

The pack source for all of it stays pristine under `template/targets/` — see
that copy for the generic "adopting this in a new project" steps.

To run one here:

1. **Author** — write a filled target as `targets/target-<slug>.md`, work log
   alongside.
2. **Gate** — get a `READY` from `target-reviewer`.
